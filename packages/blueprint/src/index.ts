...
import type {
  ...
  BLUEPRINT_ISLAND_ENTRY_SCHEMA_STATIC,
  BLUEPRINT_ISLAND_SCHEMA_STATIC,
  BLUEPRINT_SCHEMA_STATIC,
} from './schema.js';
...
// These types are resolved lazily to avoid a circular import with schema.ts.
// schema.ts imports BLUEPRINT_IDENTIFIER_REGEX and decode from this file,
// so we cannot import schema constants at module initialisation time here.
// Instead we accept the schema instance as a parameter where needed.
export type Blueprint = z.infer<typeof BLUEPRINT_SCHEMA_STATIC>;
export type BlueprintIsland = z.infer<typeof BLUEPRINT_ISLAND_SCHEMA_STATIC>;
export type BlueprintIslandEntry = z.infer<typeof BLUEPRINT_ISLAND_ENTRY_SCHEMA_STATIC>;
export type BlueprintBuilding = z.infer<typeof BLUEPRINT_BUILDING_SCHEMA_STATIC>;
...
export function update(
  ...
  blueprint.V = version;
  const identifier = encode(blueprint, schema);
  return identifier;
}
...
export function encode(
  ...
  if (!isBlueprint(value, schema)) throw new Error('Invalid blueprint');
...
export function getBuildingCount(blueprint: Blueprint): number {
  if (blueprint.BP.$type === 'Island') {
    return blueprint.BP.Entries.reduce<number>(
      (previousIsland: number, currentIsland: BlueprintIslandEntry) => {

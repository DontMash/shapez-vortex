	...
	const layers = layerIdentifiers.map<ShapeLayerData>((layerIdentifier) => {
		...
		const parts =
			partIdentifiers?.map<ShapePartData>((partIdentifier) => {
				const partShapeParameters = partIdentifier.match(SHAPE_PART_PARAMETERS_REGEX);
				const partData: ShapePartData = {
					type: partShapeParameters ? (partShapeParameters[0] as ShapeTypeIdentifier) : '-',
					color: partShapeParameters ? (partShapeParameters[1] as ShapeColorIdentifier) : '-'
				};
				return partData;
			}) ?? [];
...
export function random(options: RandomOptions): ShapeIdentifier {
	const {
		types = Math.random() < 0.5 ? SHAPE_DEFAULT_TYPE_IDENTIFIERS : SHAPE_HEX_TYPE_IDENTIFIER,
		colors = SHAPE_COLORS
	} = options;
...
function randomLayer(options: Required<RandomOptions>): ShapeIdentifier {
	...
	const partCount = isHex(types) ? SHAPE_MAX_HEX_PARTS : SHAPE_MAX_DEFAULT_PARTS;
...
function validatePart(shape: Shape, layerIndex: number, partIndex: number): Shape {
	...
	if (lowerIndex >= 0) {
		...
		if (!isPartFloating(shape, layerIndex, partIndex) && isRigid(lowerPart.type)) {
			return shape;
		}
	}
...
function simulatePart(shape: Shape, layerIndex: number, partIndex: number): Shape {
	const drop = (): Shape => {
	...
	const nextIndex = (SHAPE_MAX_DEFAULT_PARTS + partIndex + 1) % SHAPE_MAX_DEFAULT_PARTS;
	const previousIndex = (SHAPE_MAX_DEFAULT_PARTS + partIndex - 1) % SHAPE_MAX_DEFAULT_PARTS;
...
function isPartFloating(shape: Shape, layerIndex: number, partIndex: number): boolean {
	const lowerIndex = layerIndex - 1;
	if (lowerIndex < 0) {
		return false;
	}
...
function isPartConnected(shape: Shape, layerIndex: number, partIndex: number): boolean {
	const part = shape[layerIndex][partIndex];
...
export function getTypes(shape: Shape): Set<ShapeType> {
	return shape.reduce<Set<ShapeType>>((previousLayer, currentLayer) => {
		const types = currentLayer.reduce<Set<ShapeType>>((previousPart, currentPart) => {
...
export function getColors(shape: Shape): Set<ShapeColor> {
	return shape.reduce<Set<ShapeColor>>((previousLayer, currentLayer) => {
		const colors = currentLayer.reduce<Set<ShapeColor>>((previousPart, currentPart) => {
...
export function getLayerCount(shape: Shape): number {
	return shape.reduce<number>(
		(perviousLayer, currentLayer) => perviousLayer + Number(currentLayer.length > 0),
		0
	);
}

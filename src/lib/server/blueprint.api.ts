...
export const get = async (pb: PocketBase, options?: BlueprintGetOptions) => {
	const { query, sort, order, filter, page, perPage } = { ...DEFAULT_GET_OPTIONS, ...options };
	...
	const blueprints = await pb.collection<BlueprintRecord>('blueprints').getList(page, perPage, {
...
export const post = async (pb: PocketBase, value: BlueprintFormData): Promise<BlueprintRecord> => {
	...
	const record = await pb.collection<BlueprintRecord>('blueprints').create(formData);
...
export const put = async (
	...
	const blueprintRecord = await pb.collection<BlueprintRecord>('blueprints').getOne(id);
...
export const getBlueprintOptions = (url: URL) => {
	...
	const filter = filterEntries.reduce<Record<string, Array<string>>>((result, current) => {

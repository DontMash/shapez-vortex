...
export const copy: Action<HTMLButtonElement, CopyParameters> = (
  button,
  ...
  function onCopy() {
    const onSuccess = () =>
      button.dispatchEvent(
        new CustomEvent<string>('copy', { detail: copyValue }),
      );
    const onFailure = (message: string) =>
      button.dispatchEvent(
        new CustomEvent<Error>('error', { detail: new Error(message) }),
      );

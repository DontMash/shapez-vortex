<script lang="ts">
	...
	$: {
		...
		tags = data.tags?.filter((tag: BlueprintTag) => !blueprintTags.has(tag.name));
	...
	async function onSubmit() {
		...
		try {
			...
			const previewImage = new File([blob], 'preview.png', { type: 'image/png' });
...
<form class="space-y-4" method="post" enctype="multipart/form-data" use:enhance>
	<Form.Field {form} name="title">
		<Form.Control let:attrs>
			...
			<Input placeholder="My blueprint ..." {...attrs} bind:value={$formData.title} />
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
		...
		<Form.Control let:attrs>
			<Form.Label>
				...
				<Tooltip.Root>
					...
					<Tooltip.Content>
						<p>The blueprint identifier needs to be <br />in the standard format of the game.</p>
					</Tooltip.Content>
				</Tooltip.Root>
			</Form.Label>
			<Textarea
	...
	<label class="relative mt-4 block" for="blueprint-file">
		<input
			...
			on:change={async (event) => {
				...
				if (!$formData.title) $formData.title = file.name.slice(0, -BLUEPRINT_FILE_FORMAT.length);
		...
		>
			...
			<Tooltip.Root>
				...
				<Tooltip.Content>
					<p>The name and content of the file will be added to the fields above.</p>
		...
		<Form.Control let:attrs>
			<Form.Label>
				...
				<Tooltip.Root>
					...
					<Tooltip.Content>
						<p>Max. {BLUEPRINT_IMAGES_MAX} images, including preview image.</p>
						<p>Max. file size is {BLUEPRINT_IMAGE_MAX_FILE_SIZE / 1024 / 1024}MB.</p>
					</Tooltip.Content>
				</Tooltip.Root>
			</Form.Label>
			<div class="relative">
				...
				<input
					...
					on:change={(event) => {
						...
						files.slice(0, max).forEach((file) => {
							if (!BLUEPRINT_IMAGE_TYPES.find((type) => file.type === type)) return;
							data.items.add(file);
						});
	...
	<div class="space-y-2">
		<Label class="flex items-center space-x-1 lg:px-0">
			{#if data.type === 'update'}
				<Checkbox aria-labelledby="blueprint-preview-label" bind:checked={blueprintPreview} />
			{/if}
			<span>{data.type === 'update' ? 'Update blueprint preview' : 'Blueprint preview'}</span>
			<Tooltip.Root>
				<Tooltip.Trigger>
					<span class="icon-[tabler--info-circle] align-text-bottom text-lg text-input" />
				</Tooltip.Trigger>
	...
	<Button class="mt-12 w-full" type="button" aria-roledescription="submit" on:click={onSubmit}>
...
<AlertDialog.Root open={!!leaveUrl}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>Unsaved changes will be lost.</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
			<AlertDialog.Cancel on:click={() => (leaveUrl = undefined)}>Cancel</AlertDialog.Cancel>
			<AlertDialog.Action

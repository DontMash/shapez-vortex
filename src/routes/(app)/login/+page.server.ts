...
export const actions = {
    login: async ({ locals, request }) => {
        ...
        try {
            await locals.pb.collection('users').authWithPassword(result.data.username, result.data.password);
            ...
            return fail(500, { data: entries, issues: [{ message: error.message }], error: true });
        }
    },
} satisfies Actions;

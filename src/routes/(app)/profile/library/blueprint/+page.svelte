<script lang="ts">
	...
	function getBlueprintViewURL(identifier: BlueprintString): string {
		...
		{#key update}
			{#await create() then { blueprint }}
				{#await blueprint.getAll()}
					<div class="absolute left-1/2 top-0 -translate-x-1/2 items-center justify-center">
						<Loading />
					</div>
				{:then bookmarks}

...
>
  <div class="flex flex-col space-y-4 p-4 md:col-span-2">
    <header class="space-y-2 border-b border-base-content/20 pb-4">
      <div
        class="flex flex-col-reverse justify-between gap-y-2 md:flex-row md:items-center"
      >
          ...
          {#if data.user && data.user.verified && data.user.id !== data.blueprint.entry.creator}
            ...
            <Dialog bind:this={reportDialog}>
              <div class="p-6">
                ...
                <form
                  class="space-y-2"
                  action="?/reportBlueprint"
                  method="post"
                >
                  <input
                    type="hidden"
                    name="blueprint"
                    value={data.blueprint.entry.id}
                  />
          ...
          {#if data.user}
            {#if data.user.id !== data.blueprint.entry.creator}
              ...
              >
                <button
                  class="btn btn-square btn-ghost btn-sm"
                  title="{data.blueprint.isBookmarked
                    ? 'Unbookmark'
                    : 'Bookmark'} blueprint"
                >
                  {#if data.blueprint.isBookmarked}
                    <span class="icon-[tabler--bookmark-filled] text-2xl"
                      >Bookmark</span
                    >
                  {:else}
                    <span class="icon-[tabler--bookmark] text-2xl"
                      >Bookmark</span
                    >
                  {/if}
                </button>
  ...
  >
    ...
    >
      <div class="flex items-center space-x-4">
        <span
          class="icon-[tabler--square-rounded-letter-t] text-2xl text-primary"
        >
          Blueprint Type
        </span>
        <span>
      ...
      <div class="flex items-center space-x-4">
        <span class="icon-[tabler--currency-bitcoin] text-2xl text-secondary"
          >Blueprint cost</span
        >
        <span>
      ...
      <div class="flex items-center space-x-4">
        ...
        <span>
          <span class="font-medium"
            >{data.blueprint.entry.islandCount || '-'}</span
          >
          <span class="text-sm text-base-300">
      ...
      <div class="flex items-center space-x-4 md:mt-4">
        ...
        <span>
          <span class="font-medium"
            >{data.blueprint.entry.viewCount || '-'}</span
          >
          <span class="text-sm text-base-300">
      ...
      <div class="flex items-center space-x-4">
        ...
        <span>
          <span class="font-medium"
            >{data.blueprint.entry.downloadCount || '-'}</span
          >
          <span class="text-sm text-base-300">
      ...
      <div class="flex items-center space-x-4">
        ...
        <span>
          <span class="font-medium"
            >{data.blueprint.entry.bookmarkCount || '-'}</span
          >
          <span class="text-sm text-base-300">
        ...
        <div class="carousel carousel-center flex">
          {#each images as image, index}
            ...
            <div class="carousel-item relative w-full" {id}>
              ...
              >
                <span class="icon-[tabler--window-maximize] text-lg"
                  >Open detail view</span
                >
              </button>
              {#if images.length > 1}
                <div
                ...
                >
                  <a
                    class="btn btn-circle btn-neutral btn-sm shadow-md"
                    href={previousId}
                  >
                    <span class="icon-[tabler--chevron-left] text-lg"
                      >Previous</span
                    >
                  </a>
                  <a
                    ...
                    href={nextId}
                  >
                    <span class="icon-[tabler--chevron-right] text-lg"
                      >Next</span
                    >
                  </a>
                </div>

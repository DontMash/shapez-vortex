...
import { BLUEPRINT_FILE_FORMAT, type BlueprintIdentifier } from '$lib/blueprint.types';
import { getBuildingCount, getCost, decode, isBlueprintIdentifier, getIslandCount } from '$lib/server/blueprint';
...
export const load = (({ url }) => {
    const identifier = url.searchParams.get('identifier')?.trim() as BlueprintIdentifier ?? BLUEPRINT;
    ...
    try {
        ...
        const count = getBuildingCount(data);
        const meta = {
            name,
            buildingCount: count,
            islandCount: getIslandCount(data),
            cost: getCost(count)
        };

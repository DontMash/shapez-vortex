...
import { BLUEPRINT_FILE_FORMAT, type BlueprintData, type BlueprintIdentifier } from '$lib/blueprint.types';
import { decode, isBlueprintIdentifier } from '$lib/server/blueprint';
...
export const load = (async ({ locals, url }) => {
    let entry: BlueprintData | undefined = undefined;
    try {
        const blueprintId = url.searchParams.get('id')?.trim();
        if (blueprintId) {
            entry = await locals.pb.collection('blueprints').getOne<BlueprintData>(blueprintId);
        }
    } catch (err) {
        error(404, 'Blueprint not found');
    }

    const blueprintIdentifier = url.searchParams.get('identifier')?.trim() as BlueprintIdentifier ?? BLUEPRINT;
    const identifier = entry ? entry.data : blueprintIdentifier;
    if (!isBlueprintIdentifier(identifier)) error(400, 'Invalid blueprint identifier');
